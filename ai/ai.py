...
class AI(MapPool):
    ...
    def alphaBeta(self, position, current_depth, target_depth, alpha, beta, my_turn):
        if time.time() > self.timeout:
            return 0
            ...
            if my_turn:
                ...
                for move in possibleMoves:
                    ...
                    for direction in move[2]:
                        ...
                        best_score = max(best_score,
                                         self.alphaBeta(new_map, current_depth + 1, target_depth, alpha, beta, False))
                ...
                for move in possibleMoves:
                    ...
                    for direction in move[2]:
                        ...
                        best_score = min(best_score, self.alphaBeta(new_map, current_depth + 1, target_depth, alpha, beta, True))
    ...
    def bestMove(self, position, current_depth, target_depth, alpha, beta):
        if time.time() > self.timeout:
            return 0
        ...
        if current_depth == target_depth or not possibleMoves:
            ...
            for move in possibleMoves:
                ...
                for direction in move[2]:
                    ...
                    new_score = self.alphaBeta(new_map, current_depth + 1, target_depth, alpha, beta, False)
                    self.pool.give_back(max_map)
    ...
    def move(self, x1, y1, direction, position):            # Take starting coordinates of the pawn and move it in the direction mentionned.
        for key, value in directions.items():
            if direction == key:
                x2 = x1 + value[0]                          # Find the final coordinates for the pawn.
                y2 = y1 + value[1]
        ...
        for i in range(len(positions)):
            for j in range(len(positions)):
                if len(positions[i][j]) > 0:                # Check if there is a turret with minimum 1 pawn
                    lastpawn = len(positions[i][j]) - 1
                    if positions[i][j][lastpawn] == 1:      # Check if the last pawn is black
                        blackScore += 1
                    else:                                   # Or red
                        redScore += 1
        if self.pawn == 1:
